...
class Scanner {
    ...
    public TokenInfo getNextToken() {
        ...
        case '&':
            ...
            if (ch == '=') {
            	nextCh();
            	return new TokenInfo(B_AND_ASSIGN, line);
            } else if (ch == '&') {
                nextCh();
                return new TokenInfo(LAND, line);
            } else {
            	return new TokenInfo(B_AND, line);
            }
        case '^':
        	nextCh();
        	if (ch == '=') {
        		nextCh();
        		return new TokenInfo(B_XOR_ASSIGN, line);
        	} else {
        		return new TokenInfo(B_XOR, line);
        	}
        case '~':
        	nextCh();
        	return new TokenInfo(B_COMP, line);
        case '|':
        	nextCh();
        	if (ch == '=') {
        		nextCh();
        		return new TokenInfo(B_OR_ASSIGN, line);
        	} else if (ch == '|') {
        		nextCh();
        		return new TokenInfo(LOR, line);
        	} else {
        		return new TokenInfo(B_OR, line);
        	}
        case '>':
            nextCh();
            if (ch == '=') {
            	...
            	if (ch == '>') {
            		...
            		if (ch == '=') {
            			nextCh(); 
            			return new TokenInfo(UR_SHIFT_ASSIGN, line);
            		} else {
            			return new TokenInfo(UR_SHIFT, line);
            		}
            	} else if (ch == '=') {
            		nextCh();
            		return new TokenInfo(SR_SHIFT_ASSIGN, line);
            	} else {
            		nextCh();
            		return new TokenInfo(SR_SHIFT, line);
            	}
            } else {
        ...
        case '<':
            ...
            if (ch == '=') {
            	...
            	nextCh();
            	if (ch == '=') {
            		return new TokenInfo(SL_SHIFT_ASSIGN, line);
            	} else {
            		return new TokenInfo(SL_SHIFT, line);
            	}
            } else {
        ...
        case '0':
            ...
            if (ch == '.') {
            	buffer.append('.');
            	nextCh();
            	return getFloatingPointToken(buffer);
            } 
            // Handle hexa-decimals
            else if (ch == 'X' || ch == 'x') {
            	buffer.append('x');
            	nextCh();
            	while (isHex(ch)) {
        ...
        case '9':
            ...
            if (ch == 'E' || ch == 'e') {
            	buffer.append('e');
            	nextCh();
            	return getExponentialDouble(buffer);
            } else if (ch == '.') {
            	buffer.append(ch);
            	nextCh();
            	return getFloatingPointToken(buffer);
    ...
    private TokenInfo getFloatingPointToken(StringBuffer buffer) {
    	while (isDigit(ch)) {
    		buffer.append(ch);
            nextCh();
    	}
    ...
    private TokenInfo getExponentialDouble(StringBuffer buffer) {
