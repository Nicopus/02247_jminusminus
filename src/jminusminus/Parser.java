...
public class Parser {
	...
	private boolean seeForEachStatement() {
		scanner.recordPosition();
		if (seeBasicType() || seeReferenceType()) {
			scanner.next();
			if (have(IDENTIFIER) && have(COL)) {
				scanner.returnToPosition();
				return true;
			}
		}
	...
	private JAST typeDeclaration() {
		...
		if (see(CLASS)) {
			...
			return interfaceDecleration(mods);
		}
	}
	...
	private JInterfaceDecleration interfaceDecleration(ArrayList<String> mods) {
		int line = scanner.token().line();
		mustBe(INTERFACE);
		mustBe(IDENTIFIER);
		String name = scanner.previousToken().image();
		ArrayList<Type> superClasses = new ArrayList<>();
		if (have(EXTENDS)) {
			...
			superClasses.add(Type.NULLTYPE);
		}
		return new JInterfaceDecleration(line, mods, name, superClasses, interfaceBody());
	}
	...
	private JMember memberDecl(ArrayList<String> mods) {
		...
		if (seeIdentLParen()) {
			...
			mustBe(RCURLY);
			memberDecl = new JInitializationBlock(line, statements, mods.contains("static"));
		} else {
			Type type = null;
			if (have(VOID)) {
				// void method
	...
	private JStatement statement() {
		...
		if (see(LCURLY)) {
			...
			if (seeForEachStatement()) {
				...
				mustBe(SEMI);
				JStatement uexpr = statementExpression();
				mustBe(RPAREN);
				JStatement body = statement();
				return new JForLoopStatement(line, vdecl, cexpr, uexpr, body);
			}
		} else if (have(TRY)) {
			JBlock body = block();
			ArrayList<JFormalParameter> cParams = new ArrayList<>();
			ArrayList<JBlock> cBlocks = new ArrayList<>();
			JBlock fBlock = null;
			while (have(CATCH)) {
				mustBe(LPAREN);
				cParams.add(formalParameter());
				mustBe(RPAREN);
				cBlocks.add(block());
			}
			if (have(FINALLY)) {
			...
			return new JTryBlockStatement(line, body, cParams, cBlocks, fBlock);
		} else if (have(THROW)) {
	 ...
	 * 
	 * <pre>
	 * // level 12 conditionalExpression ::= conditionalOrExpression [CONDITIONAL
	 * assignmentExpression COL conditionalExpression]
	 * 
	 * <pre>
	 * 
	...
	private JExpression conditionalExpression() {
		...
		if (have(CONDITIONAL)) {
			JExpression expr1 = assignmentExpression();
			mustBe(COL);
			JExpression expr2 = conditionalExpression();
			return new JConditionalExpression(line, lhs, expr1, expr2);
		} else {
			return lhs;
		}
	}
	 ...
	 * 
	 * <pre>
	 * // level 11 conditionalOrExpression ::= conditionalAndExpression { LOR
	 ...
	 * 
	 * <pre>
	 * // level 9 inclusiveOrExpression ::= exclusiveOrExpression {B_OR
	 ...
	 * 
	 * <pre>
	 * // level 8 exclusiveOrExpression ::= andExpression {B_XOR andExpression}
	 * 
	 * <pre>
	 * 
	 * @return
	 ...
	 * 
	 * <pre>
	 * // level 7 andExpression ::= equalityExpression {B_AND equalityExpression}
	 * 
	 * <pre>
	 * 
	 * @return
	 ...
	 * 
	 * <pre>
	 * // level 4 shiftExpression ::= additiveExpression {(SL_SHIFT< | SR_SHIFT |
	 * UR_SHIFT) additiveExpression}
	 * 
	 * <pre>
	 *
	...
	private JExpression shiftExpression() {
		...
		while (more) {
			if (have(SL_SHIFT)) {
				lhs = new JSignedLeftShiftOp(line, lhs, additiveExpression());
			} else if (have(SR_SHIFT)) {
				lhs = new JSignedRightShiftOp(line, lhs, additiveExpression());
			} else if (have(UR_SHIFT)) {
